function photoswipe_init()
{
    ...
    var _list   = { bgOpacity : 1, dataSource : [], showHideAnimationType : "none" };
    var object  = null;
    var _object = null;
    var item    = null;
    var href    = "";
    ...
    var func    = function()
    {
        _list.index = parseInt(this.getAttribute("data-photoswipe-index"));
        console.log(_list);
        new st(_list).init();
        return false;
    };
    ...
    for (var i = 0; i < c; i++)
    {
        ...
        {
            if (! object.hasAttribute("href")) continue;
            href = object.getAttribute("href");
            for (var _i = 0; (_i < _list.dataSource.length) && (_list.dataSource[_i].src != href); _i++);

            if (_i == _list.dataSource.length)
            {
                if (! object.hasAttribute("data-size")) continue;
                size = object.getAttribute("data-size").split("x");
                ...
                _object = object.getElementsByTagName("img");
                if (_object.length > 0)
                {
                    _object = _object[0];
                    if (_object.hasAttribute("src"))
                        item.msrc   = _object.getAttribute("src");
                    if (_object.hasAttribute("srcset"))
                        item.srcset = _object.getAttribute("srcset");
                    if (_object.hasAttribute("alt"))
                        item.title  = _object.getAttribute("alt");
                };

                _list.dataSource.push(item);
            };

            object.setAttribute("data-photoswipe-index", _i);
            object.onclick = func;
        };
    };
};
...
export{st as PhotoSwipe,photoswipe_init};